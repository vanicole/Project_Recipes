...
RSpec.describe Recipe, type: :model do
  ...
  it "Create a recipe with step too short" do
    user = User.create(:email => "user@test.it", :password => "12345678")
    recipe = Recipe.create(:title => "Titolo", :category => "Primo piatto", :dosage => "2 persone", :difficulty => "Alta", :preparation => "20 minuti", :ingredients => "Vari", :steps => "Low", :user_id => "#{user.id}")
    expect(recipe).not_to be_valid
  end
